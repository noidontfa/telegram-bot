...
def translate(update: Update, context: CallbackContext):
    args = context.args
    if not len(args):
        update.message.reply_text(f"""Invalid command: no input""")
        return
    text = ' '.join(args)
    user_id = update.message.chat.id
    translation_default = services.get_translation_default(user_id=user_id)
    translation = _translate(text, src=translation_default.src, dest=translation_default.dest)
...
def set_translation_default(update: Update, context: CallbackContext):
    ...
    src = context.args[0]
    dest = context.args[1]
    translation_default = services.set_translation_default(user_id=user_id, src=src, dest=dest)
    table = pt.PrettyTable(['Source', 'Destination'])
    table.add_row([f'{translation_default.src_name} / {translation_default.src}',
